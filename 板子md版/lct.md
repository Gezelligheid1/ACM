...
#define rs son[x][1]
    ...
    void rotate(int x)
    {
        ...
        if (!is_root(y))son[z][son[z][1] == y] = x;//一定要判断,注意是is_root(y)!!!
    ...
    void split(int x, int y)//把x-y的路径变成一个splay,这里再把y变成splay的根
    {
        makeroot(x);
        access(y), splay(y);
    }
...
int main()
{
    ...
    while (m--)
    {
        ...
        if (op == 0)
        {
            ...
            t.splay(x), t.c[x].v = y;//别忘记splay!!! 查询时由于split会执行pushup所以此处不需要pushup
    }
    return 0;
}
```