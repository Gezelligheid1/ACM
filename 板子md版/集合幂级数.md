<div align = "center">集合幂级数</div>

$$
\begin{aligned}
&\  \ \ \ \  \ \ \ \ \ \ \ \prod_{i=1}^n(1+x^{a_i})\\
&=exp\sum_{i=1}^nln(1+x^{ai})\\
&=exp\sum_{i=1}^n\sum_{k=1} \frac{(-1)^{k+1}(x^{a_i})^k}{k}\\
&=exp\sum_{i=1}^nx^{a_i}\ \ \ \ \ \ 因为是子集卷积
\end{aligned}
$$

```c++
namespace polybase {//注意是int，conv的n是指数，别忘记清空c，封装的函数都带拷贝，一次性的可以手动写
    constexpr int mod = 998244353, L = 20;//注意模数和L大小
    ...
    void ln(int A[][1 << L], int b[][1 << L], int n)
    {
        for (int i = 0; i <= n; i++)
            for (int j = 0; j < 1 << n; j++)
                a[i][j] = A[i][j];
        ...
        for (int k = 0; k < 1 << n; k++)
        {
            ...
            for (int i = 1; i <= n; i++)
            {
                ...
                for (int j = 1; j <= i - 1; j++)//i-1
                    sum = (sum + (ll) j * b[j][k] % mod * a[i - j][k]) % mod;
                b[i][k] = (a[i][k] - sum * inv[i] % mod + mod) % mod;
            }
        }
    ...
    void inverse(int A[][1 << L], int b[][1 << L], int n)
    {
        for (int i = 0; i <= n; i++)
            for (int j = 0; j < 1 << n; j++)
                a[i][j] = A[i][j];
