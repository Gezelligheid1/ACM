<div align = "center">exlucas 模数不固定 适合使用次数较少</div>
...
namespace exlucas {
    ...
    ll exlucas(ll n, ll m, ll P)//根号P预处理
    {
        vector<ll> R, M;
        for (ll i = 2; i * i <= P; i++)
        {
            ...
            while (P % i == 0)
                P /= i, tmp *= i;
            R.emplace_back(Binom(n, m, i, tmp));
            M.emplace_back(tmp);
        }
        if (P > 1)
            R.emplace_back(Binom(n, m, P, P)), M.emplace_back(P);
        return crt(R, M);
    }
}
```