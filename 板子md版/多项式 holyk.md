<div align = "center">多项式 Holyk</div>
...
namespace polybase {
    ...
    {
        w[L / 2] = 1;
        for (int i = L / 2 + 1, x = fpow(g, (mod - 1) / L); i < L; i++)
            w[i] = 1LL * w[i - 1] * x % mod;
...
#define T (*this)
        ...
        {
            if (r.len() < len()) resize(r.len());
            for (int i = 0; i < len(); i++) T[i] = 1LL * T[i] * r[i] % mod;
            return T;
        }
        ...
        poly deriv() const
        {
            ...
            poly r(len() - 1);
            for (int i = 1; i < len(); i++) r[i - 1] = 1LL * i * T[i] % mod;
            return r;
        }
        ...
        poly integ() const
        {
            if (empty()) return poly();
            poly r(len() + 1);
            for (int i = 0; i < len(); i++) r[i + 1] = 1LL * fpow(i + 1) * T[i] % mod;
            return r;
        }
        ...
        poly ln(int m) const { return (deriv() * inv(m)).integ().pre(m); }
        ...
        poly exp(int m) const
        {
            ...
            for (int k = 1; k < m; k *= 2)
            {
                x.append((x * (pre(k * 2) - x.ln(k * 2) >> k)).pre(k));
            }
            return x.pre(m);
        }
        ...
        poly sqrt(int m) const
        {
            ...
            for (int k = 1; k < m; k *= 2)
            {
                x.append(((pre(k * 2) - x * x >> k) * y).pre(k) * (mod + 1 >> 1));
                if (k * 2 < m)
                {
